...
function filterUsers(data, hasCar) {
  const clonedData = [];
  data.forEach((company) => {
    const clonedCompany = {
      ...company,
    };
    const filteredUsers = clonedCompany.users.filter((user) => user.car === hasCar);
    clonedCompany.users = filteredUsers;
    clonedCompany.usersLength = filteredUsers.length;
    clonedData.push(clonedCompany);
  });
  return clonedData;
}

console.log('---- EXAMPLE 2 --- ', filterUsers(companies, false));
