...
function convertCompanies(data) {
  return data.flatMap((company) => {
    return company.users.map((user) => {
      const newData = {};
      const attributeName = `${user.lastName}${user.firstName}${user.age}`;
      newData[attributeName] = user.car;
      return newData;
    });
  });
}

console.log('---- EXAMPLE 6 --- ', convertCompanies(companies));
